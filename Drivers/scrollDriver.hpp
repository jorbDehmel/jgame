#ifndef SCROLLDRIVER_H
#define SCROLLDRIVER_H
...
public:
    ...
    set<SPRITE_TYPE> scrollExemptTypes;
};
...
ScrollDriver::ScrollDriver(Stage *stageIn, double overScrollRatioX, double overScrollRatioY)
{
    ...
    scrollExemptTypes.insert(PLAYER);
    scrollExemptTypes.insert(LABEL);
...
void ScrollDriver::handle()
{
    ...
    else if (KEYS['s'] || KEYS[keys::downArrow])
    {
        if (player->y() < defaultY)
            ...
            yChange -= yInc;
        else if (player->y() + player->h() < renderH)
            player->yInc(yInc);
    }

    if (KEYS['d'] || KEYS[keys::rightArrow])
    {
        if (player->x() < defaultX)
            player->xInc(xInc);
        else if (xOffset > minX)
            xChange -= xInc;
        else if (player->x() + player->w() < renderW)
            player->xInc(xInc);
    }
    ...
    {
        if (scrollExemptTypes.find(i->type) == scrollExemptTypes.end())
        {
            i->rect.x += xChange;
            i->rect.y += yChange;
        }
